...
class OfertasController extends \BaseController {
    ...
    public function expositoresporrubro($app,$id_rubro)
    {
        ...
        {
            $id_sistema = $sistemas[0]["id"];
            $query = DB::connection('Pilar')->select("SELECT rubro_id, rubro, expositor_id, expositor, link, empresa  FROM ofertas WHERE sistema_id=".$id_sistema." AND rubro_id=".$id_rubro." ORDER BY expositor");
            if (sizeof($query))
            {
                $resultado=array();
                foreach ($query as $data) {
